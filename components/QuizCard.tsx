...
const QuizCard: React.FC<QuizCardProps> = ({ quiz, icon, delay = 0 }) => {
  ...
  return (
    <motion.div
      {...fadeIn}
      transition={{ delay }}
      className={`bg-gradient-to-br ${getTypeGradient(quiz.type)} p-6 rounded-2xl border border-purple-500 hover:border-purple-400 transition-all duration-300 cursor-pointer transform hover:scale-105 group`}
    >
      {/* Header */}
      <div className="flex items-center justify-between mb-4">
        ...
        <div
          className={`px-3 py-1 rounded-full text-xs font-semibold ${getDifficultyColor(quiz.difficulty)}`}
        >
          {quiz.difficulty}
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {quiz.tags.slice(0, 3).map((tag: string, index: number) => (
          <span
            key={index}
            className="bg-black bg-opacity-30 text-purple-200 px-2 py-1 rounded-full text-xs"
          >
            #{tag}
          </span>
        ))}
